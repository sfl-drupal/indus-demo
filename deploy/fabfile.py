...
def drush_commands(role='docker', cmds=POST_INSTALL):
    ...
    if not LOCALE and 'drush po-import fr --custom-only' in cmds:
...
def docker_clean_all():
    ...
    """
    execute(delete_root)
    execute(docker_stop_container)
    execute(docker_remove_container)
    execute(docker_remove_image)
    print green('Clean all finished with success!')